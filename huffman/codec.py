...
class Node:
    def __init__(self, NodeL : 'Node', NodeR : 'Node'):
        self.NodeL = NodeL
        self.NodeR = NodeR
        self.string = NodeL.string + NodeR.string # a Node weight is the sum of its children's
...
class Codec:
    ...
    def encode_bin(self, text : str):
        ...
        if encoded[(fin+1)*8:] == '' :
            ...
            longueur = len(encoded[(fin+1)*8:]) # On enregistre de la dernière découpe pour
            Numbers.append(number)             #Pouvoir reconstruire le message
            Numbers.append(longueur)
        encoded_bin = pack('B'*len(Numbers),*Numbers)
        return encoded_bin

    def decode_bin(self, encoded_bin : str):
...
encoded  = codec.encode_bin('abdeab')
print(encoded)
